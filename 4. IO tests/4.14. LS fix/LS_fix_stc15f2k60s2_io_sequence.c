...
sfr P0M1    = 0x93;   /* PxM1.n, PxM0.n = 00 -> quasi-bidirectional  */
sfr P0M0    = 0x94;   /*                 = 01 -> push-pull output     */
sfr P1M1    = 0x91;   /*                 = 10 -> high-impedance input */
sfr P1M0    = 0x92;   /*                 = 11 -> open-drain           */
sfr P2M1    = 0x95;
sfr P2M0    = 0x96;
sfr P3M1    = 0xB1;
sfr P3M0    = 0xB2;
...
sbit ZOOMER  = P2^6;   /* Buzzer:                   P2.6 */
sbit P1_3    = P1^3;   /* Spare push-pull output,   P1.3 (driven HIGH) */

sbit BTN_UP   = P5^5;  /* Push button UP:    P5.5 */
sbit BTN_DOWN = P4^0;  /* Push button DOWN:  P4.0 */
sbit LS_UP    = P1^4;  /* Upper limit switch: P1.4 (high-Z, external pull-up) */
sbit LS_DOWN  = P1^5;  /* Lower limit switch: P1.5 (high-Z, external pull-up) */
 ...
 *
 * Pin assignments:
 *   P0.2  -> push-pull output, LOW  (unknown function)
 *   P0.3  -> push-pull output, HIGH (unknown function)
 *   P1.0  -> ADC input  (current sense, analog)
 *   P1.1  -> ADC input  (supply voltage, analog)
 *   P1.2  -> OUTPUT3    (motor DOWN, push-pull)
 *   P1.3  -> P1_3       (spare, push-pull HIGH)
 *   P1.4  -> LS_UP      (upper limit switch, high-Z, external pull-up)
 *   P1.5  -> LS_DOWN    (lower limit switch, high-Z, external pull-up)
 *   P2.4  -> push-pull output, HIGH (unknown function)
 *   P2.5  -> push-pull output, HIGH (unknown function)
 *   P2.6  -> ZOOMER     (buzzer, push-pull)
 ...
 *   P4.2  -> push-pull output, LOW (unknown function)
 *   P4.3  -> push-pull output, LOW (unknown function)
...
static void uart_init(void)
{
    ...
    /* P1ASF: P1.0/P1.1 = ADC; P1.2-P1.7 = digital (clears ADC mode on P1.4/P1.5) */
    P1ASF = 0x03;

    /* P0.2 push-pull LOW, P0.3 push-pull HIGH (unknown function) */
    P0M1 &= (unsigned char)~0x0C;
    P0M0 |= 0x0C;
    ...
    /* P1.3 push-pull (spare, driven HIGH) */
    P1M1 &= (unsigned char)~0x08;
    P1M0 |= 0x08;

    /* P1.4, P1.5 high-Z inputs (LS_UP, LS_DOWN - rely on external PCB pull-ups) */
    P1M1 |= 0x30;
    P1M0 &= (unsigned char)~0x30;

    /* P2.4, P2.5 push-pull (unknown function, HIGH) */
    P2M1 &= (unsigned char)~0x30;
    P2M0 |= 0x30;

    /* P2.6 push-pull (ZOOMER) */
    P2M1 &= (unsigned char)~0x40;
    P2M0 |= 0x40;
    ...
    /* P4.2, P4.3 push-pull (unknown function) */
    P4M1 &= (unsigned char)~0x0C;
    P4M0 |= 0x0C;
...
void main(void)
{
    ...
    /* Initial latch values matching original firmware */
    P0  = (unsigned char)((P0 & (unsigned char)~0x04) | 0x08); /* P0.2=LOW, P0.3=HIGH */
    P1_3   = 1;     /* spare push-pull output HIGH */
    P2    |= 0x30;  /* P2.4, P2.5 HIGH (unknown function) */
    ZOOMER = 0;     /* buzzer OFF */
    BTN_UP   = 1;
    BTN_DOWN = 1;
